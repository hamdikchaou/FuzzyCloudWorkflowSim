...
public class Runtime extends RuntimeAlgorithm {
    public Runtime(BuildTimeAlgorithm aBuilder) {
        super(aBuilder);
    }
            ...
            for(int i = 0; i < lReadyTasks.size(); ++i) {
                ...
                try {
                    ...
                    for(int j = 0; j < lNewData.size(); ++j) {
                        DataCenter lTargetCenter = this.distributeNewData((DataSet)lNewData.get(j), bestCenter);
                        System.out.println(((DataSet)lNewData.get(j)).getName() + " stored in " + lTargetCenter);
