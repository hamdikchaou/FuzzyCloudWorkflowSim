...
public class FuzzyBuildTime extends BuildTimeAlgorithm {
    private Matrix fDependancyMatrix;
    private Matrix fClusteredMatrix;
    private Clusterer fClusterer = new BEA();
    private int cluster;

    public FuzzyBuildTime(ArrayList<DataCenter> aDatacenters) {
        super(aDatacenters);
    }

    public FuzzyBuildTime(ArrayList<DataCenter> aDatacenters, Matrix aMatrix, int cluster) {
        super(aDatacenters);
        this.fClusteredMatrix = aMatrix;
        this.cluster = cluster;
    }

    public void tostring() {
        System.out.println("Display Matrix from FuzzyBuildTime");
    ...
    public ArrayList<DataCenter> distribute(Workflow aWorkflow) throws DistributionException {
        System.out.println("-->Distribution_from BuildTime Classfuzyyyyyyyyyy");
        this.fDependancyMatrix = new Matrix(aWorkflow);
        System.out.println("Matrix dependencies");
        System.out.println(this.fDependancyMatrix.toString());
    ...
    private void partitionAndAssign(Matrix aMatrix, ArrayList<DataCenter> aDatacenters) throws Exception {
        if (aMatrix.getDatasets().size() == 0) {
            ...
            for(int i = 1; i < aMatrix.getDatasets().size(); ++i) {
                ...
                System.out.println("The value of PM= " + thisPointScore);
    ...
    private void assignDatasets(Matrix aMatrix, ArrayList<DataCenter> aDatacenters, int startPoint, int endPoint) throws Exception {
        ...
        for(int i = startPoint; i <= endPoint; ++i) {
            ...
            System.out.println("Dataset N� " + aMatrix.getDatasets().get(i) + "=" + ((DataSet)aMatrix.getDatasets().get(i)).getSize());
        }
