...
public class FuzzyRuntimePSO extends RuntimeAlgorithm {
    public FuzzyRuntimePSO(BuildTimeAlgorithm aBuilder) {
        super(aBuilder);
    }
    ...
    public void run(ArrayList<DataCenter> aDatacenters, Workflow aWorkflow) throws DistributionException {
        ...
        ArrayList<Task> PSOReadyTasks = this.getReadyTasks();
        System.out.println("***********************PSO start******************************");
        PSO pso = new PSO(PSOReadyTasks, this.fUsedDatacenters);
        pso.setting();
        double[] b = pso.getpsoAnswer();
        ArrayList<DataCenter> PSObestcenter = new ArrayList();

        int xy;
        String lDatasetsSent;
        for(xy = 0; xy < this.fTasks.size(); ++xy) {
            long d = Math.round(b[xy]);
            lDatasetsSent = "dc" + d;

            for(int z = 0; z < this.fDatacenters.size(); ++z) {
                if (((DataCenter)this.fDatacenters.get(z)).getName().equals(lDatasetsSent)) {
                    PSObestcenter.add((DataCenter)this.fDatacenters.get(z));
                }
            }
        }
        ...
        for(xy = 0; xy < this.fTasks.size(); ++xy) {
            System.out.print(" | " + PSObestcenter.get(xy));
        }

        for(xy = 1; this.fTasks.size() > 0; this.fReport = this.fReport + "\r\n") {
            ...
            for(int i = 0; i < lReadyTasks.size(); ++i) {
                ...
                bestCenter = (DataCenter)PSObestcenter.get(i);
                ...
                try {
                    ...
                    System.out.println("ERROR : " + var35.getMessage());
                    if (var35 instanceof DistributionException) {
                        throw new DistributionException(var35.getMessage());
                    }
